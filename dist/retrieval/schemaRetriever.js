...
exports.initializeSchemaEmbeddings = initializeSchemaEmbeddings;
exports.retrieveRelevantSchema = retrieveRelevantSchema;
exports.getSchemaByName = getSchemaByName;
exports.listAllSchemas = listAllSchemas;
 ...
 */
async function initializeSchemaEmbeddings() {
    console.log("Initializing schema embeddings...");
    for (const doc of schemaDocs) {
        if (!doc.embedding) {
            doc.embedding = await (0, embeddingService_1.embed)(doc.description);
        }
    }
 ...
 */
async function retrieveRelevantSchema(query, topK = 1) {
    ...
    const queryEmbedding = await (0, embeddingService_1.embed)(query);
    // Calculate similarity scores
    const rankedSchemas = schemaDocs
        .map(doc => ({
        ...doc,
        score: (0, embeddingService_1.cosineSimilarity)(queryEmbedding, doc.embedding)
    }))
