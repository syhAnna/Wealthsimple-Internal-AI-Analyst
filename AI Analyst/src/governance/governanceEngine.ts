...
import { ParsedIntent, GovernanceResult, RiskFlag } from "../types";
 ...
 */
function checkPIIAccess(intent: ParsedIntent, schema: string): RiskFlag[] {
 ...
 */
function checkFinancialSensitivity(intent: ParsedIntent, schema: string): RiskFlag[] {
  const flags: RiskFlag[] = [];
 ...
 */
function checkCompliance(intent: ParsedIntent, sql: string): RiskFlag[] {
  const flags: RiskFlag[] = [];
 ...
 */
function checkPerformanceRisk(intent: ParsedIntent, sql: string): RiskFlag[] {
  const flags: RiskFlag[] = [];
 ...
 */
function calculateConfidence(intent: ParsedIntent, flags: RiskFlag[]): number {
  let score = 1.0;
 ...
 */
function shouldApprove(flags: RiskFlag[], confidence: number): boolean {
 ...
 */
function hasTimeFilter(intent: ParsedIntent): boolean {
  if (intent.timeRange) {
    return true;
  }
  ...
  return intent.filters.some(f =>
    timeFields.some(tf => f.field.toLowerCase().includes(tf))
  );
}
 ...
 */
function requiresTimeFilter(metric: string): boolean {
